...
class TestCase extends Orchestra
{
    ...
    public function expectRouteRegistered(
        ...
        $routeRegistered = collect($this->getRouteCollection()->getRoutes())
            ->contains(function (Route $route) use ($name, $middleware, $withoutMiddleware, $controllerMethod, $controller, $uri, $httpMethods, $domain, $wheres, $isFallback, $enforcesScopedBindings, $preventsScopedBindings, $defaults, $withTrashed): bool {
                foreach (Arr::wrap($httpMethods) as $httpMethod) {
                    if (!in_array(mb_strtoupper($httpMethod), $route->methods, true)) {
                        return false;
                    }
                }
                ...
                $routeController = $route->getAction(0) ?? $route->getController()::class;
                ...
                return $route->allowsTrashedBindings() === $withTrashed;
            });
    ...
    protected function defineEnvironment($app)
    {
        ...
        $app->make(Repository::class)->set('filesystems.disks.local.serve', false);
